    ...
    def adicionar_voo(self, voo):
        for v in self.voos:
            if v.numero == voo.numero:
                print(
                    f"ERRO: O voo com número {voo.numero} já está cadastrado!")
                return
...
class Voo:
    ...
    def adicionar_passagem(self, passageiro, classe):
        ...
        print(
            f'Passagem registrada para {passageiro.nome} no voo {self.numero} com destino: {self.destino}.')
        print('-' * 70)
    ...
    def exibir_detalhes(self):
        ...
        print(
            f'Total arrecadado para este voo: R$ {self.calcular_total_arrecadado():.2f}')
...
class ClasseEconomica(ClasseVoo):  # herda da classeVoo que é a superclasse
    def __init__(self, preco, bagagem_incluida):
        # pra chamar os metodos da classe pai. Sem ele teria que reescrever a logica pra poder usar o preco aqui
...
def menu_interativo(companhia):
    while True:
        ...
        if opcao == "1":
            ...
            numero_voo = input(
                "Informe o número do voo para ver os detalhes: ")
            voo_encontrado = None
            for voo in companhia.voos:
                if voo.numero == numero_voo:
                    voo_encontrado = voo
